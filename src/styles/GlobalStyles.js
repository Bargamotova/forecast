...
import Architext from '../assets/fonts/architext.regular.ttf';
import ShellBold from '../assets/fonts/SnellBT-Bold.otf';
import ShellRegular from '../assets/fonts/SnellBT-Regular.otf';
...
:root {
 ...
 --grad_yellow: rgba(242, 154, 6, 0.5); 
  --blue-transparent: rgba(169, 223, 255, .4);
  --blue: #a9e0ff;
  --yellow: #fff0b1;
  --mint: #d3fcb3;
  --shadow: 6px 5px 10px rgba(120, 120, 120, 0.5);
  --border-thin: 2px solid #fff;
  --border-radius: 10px;
  --gradient_two_clr: linear-gradient(
    90deg,
    var(--grad_purple),
    var(--grad_blue)
  )
...
@font-face {
  ...
  src: url(${Architext}) format("truetype");
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}
...
html {
  ...
  scrollbar-color: var(--grad_purple) transparent;
  transition: all 0.5s;
}
::selection {
  background-color: var(--grad_purple);
}
body {
  min-height: 100vh;
  min-height: 100dvh;
...
body&::after {
    ...
    right: 0;
    background:
    linear-gradient(90deg, var(--grad_purple), var(--grad_blue), var(--grad_yellow)),
